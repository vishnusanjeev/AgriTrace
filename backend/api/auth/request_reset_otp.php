...
if ($purpose === 'VERIFY_EMAIL') {
    ...
    // RESET_PASSWORD: do not reveal whether the account exists
    $st = $pdo->prepare("SELECT id FROM users WHERE email_norm=? LIMIT 1");
    $st->execute([$email]);
    $u = $st->fetch();
    if (!$u) {
        json_out(['ok'=>true,'message'=>'If an account exists for this email, a reset code has been sent.']);
    }
    $uid = (int)$u['id'];
}

// rate limit
if ($pending && $purpose === 'VERIFY_EMAIL') {
